...
export const changePassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await sendEmailService("changePassword", emailParams);
...
export const resetPasswordRequest = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await sendEmailService("resetPasswordRequest", emailParams);
    return res.status(200).send({ params: emailParams , success: true });
  } catch (error: Error | any) {
    return res.status(500).json({ message: error.message });
  }
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await sendEmailService("resetPassword", emailParams);
    return res.status(200).send({ params: emailParams , success: true });
  } catch (error: Error | any) {
    return res.status(500).json({ message: error.message });
  }
